...
var rateUrl = getBrowserStoreUrl();
...
function isEasylistBlock(details, topHostAddress) {
	if (topHostAddress?.includes('yahoo.com') &&
		details.url.startsWith('https://players.brightcove.net')) {
		return false
	}
...
function onMessage(request, sender, callback) {
	...
	try {
		switch (request.type) {
			...
			case stndz.messages.setPageDataCustomCss: {
				let tabId = sender?.tab?.id;
				if (typeof tabId !== 'number') {
					return;
				}
				
				let pageData = pageDatas[tabId];
				
				if (pageData) {
					pageData.customCss = request.customCss;
				}
				break;
			}
		}
...
function getHostSettings(host) {
	...
	while (true) {
		...
		const hostSelectors = customCssRules.hosts[host] ?? [];
		if (response.customCss == null && hostSelectors.length > 0) {
			let customCss = hostSelectors.join(',');
